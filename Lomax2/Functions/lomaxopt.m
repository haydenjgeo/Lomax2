function [A_opt,B_opt] = lomaxopt(X,R,P,varargin)
...
okargs={'FitMethod' 'Censoring' 'Conditions' 'AddVars' 'MaxIter' 'MaxFunEvals' 'Display' 'theta'};
defaults={'linear' [] [] [] 100000 1000000 'off' 0};
[fitmethod, cens, conditions, addvars, maxiter, maxfunevals, display, theta]=internal.stats.parseArgs(okargs, defaults, varargin{:});
...
f = @(v) ssepareto(v,X,R,'censoring',cens, 'fitmethod',fitmethod, 'theta',theta, 'Conditions',conditions, 'AddVars',{'P' P});

vopt = fminsearch(f, [A_pre B_pre]); % use previous rough fits as start value 
A_opt=vopt(1);
B_opt=vopt(2);
